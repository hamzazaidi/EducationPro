...
export const reducer = createReducer(
  ...
  on(
    ...
    (state: QuizState, { payload }): QuizState => {
      return {
        ...state,
        categories: {
          items: [...payload],
  ...
  on(
    QuestionActions.LoadQuestionsFailure,
    (state: QuizState): QuizState => {
      return {
        ...state,
        quizStore: {
  ...
  on(
    QuizActions.ToggleRevealAnswer,
    (state: QuizState): QuizState => {
      const question: Question = state.quizStore.items.find(
        (q) => q.key === state.questionOnScreen.key
      );
      ...
      return {
        ...
        questionOnScreen: {
          ...
          answers: state.questionOnScreen.answers.map((a) => ({
            ...a,
            isCorrect: a.key === validAnswer.key,
          })),
        },
  ...
  on(
    ...
    (state: QuizState, { payload }): QuizState => {
      return {
        ...
        questionOnScreen: {
          ...state.questionOnScreen,
          answers: state.questionOnScreen.answers.map((a, i) => ({
            ...a,
            selected: a.key === payload.key,
          })),
        },
      };
    }
  ...
  on(
    QuizActions.SetQuestionOnScreen,
    (state: QuizState): QuizState => {
      ...
      if (!state.questionOnScreen.key) {
        ...
        const index = state.quizStore.items.findIndex(
          (item) => item.key === state.questionOnScreen.key
        );
  ...
  on(
    QuizActions.SaveResultsFailure,
    (state: QuizState): QuizState => {
      return {
        ...state,
        savingResult: false,
